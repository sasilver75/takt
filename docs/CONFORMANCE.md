...
| CLI workflow path, port override, and startup failure surfacing | `src/cli.ts`, `src/service.ts` | `src/cli.test.ts` |
...
| Worker evidence manifest, sticky PR evidence comment, and dashboard/API evidence summary | `src/github/evidence.ts`, `src/orchestrator/orchestrator.ts`, `src/http/server.ts` | `src/github/evidence.test.ts`, `src/orchestrator/orchestrator.test.ts`, `src/http/server.test.ts`, live PRs `#3` and `#4` |
