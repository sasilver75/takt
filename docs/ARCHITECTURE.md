...
- PR lifecycle tracking: `src/github/tracker.ts`
  - Inspects published PRs, head check runs, reviews, and inline review comments so the orchestrator can decide whether to wait for humans, requeue worker follow-up, or record terminal PR state.
...
- GitHub publishing and inspection credentials stay in the orchestrator. Workers produce commits plus a PR-ready manifest; Symphony owns branch push, PR creation/update, PR status/review inspection, Linear PR comments, review-state transition, and follow-up requeue decisions.
