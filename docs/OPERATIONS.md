...
- Agent-side Linear actions should use the Takt-owned `linear_graphql` tool exposed by the `takt_linear` MCP server. Takt hosts that MCP server on a short-lived Streamable HTTP endpoint and registers only its runtime URL with Codex; no Linear API key or bridge token is written into the worker workspace. Docker workers use an MCP bearer-token env var rather than an argv or workspace-file secret. The hosted MCP bridge and direct dynamic-tool fallback both reject multi-operation GraphQL documents before executor access and redact configured secret values from tool result payloads before returning them to the worker.
...
2. The worker implements, verifies, commits, and writes `github.pr_ready_file` in the workspace root. The PR-ready and evidence manifest files are handoff control files, not repository artifacts; they must remain uncommitted. Takt rejects PR publishing if either handoff manifest is tracked by git. The PR-ready manifest must be a JSON object with a non-empty `summary` or `body`, a non-empty `verification` string list, and a non-empty `risk` field.
...
- `GET /api/v1/state` returns target metadata, running sessions, retry queue, latest dispatch decisions, published PR status, token/runtime totals, and rate limits.
- `GET /api/v1/<issue_identifier>` returns issue-specific debug state, including the bounded per-issue worker run-attempt ledger and any latest dispatch decision explaining why the issue was skipped.
...
The publication-ledger canary is mutating and separately gated. By default it runs a failpoint matrix across branch push, PR publication, evidence artifact upload, evidence comment publication, Linear PR comment publication, and review-state handoff. Each case creates a dedicated Linear issue, commits a canary file in a temporary workspace, performs the side effect up to the selected failpoint, then restarts reconciliation from durable state to finish the PR lifecycle.
