...
- `src/github`: orchestrator-owned branch push, GitHub PR creation/update, and PR lifecycle inspection.
...
When `github.enabled` is configured, workers commit their code and signal PR readiness with `SYMPHONY_PR_READY.json`; Symphony then pushes the issue branch, creates or updates the GitHub PR, comments the PR link in Linear, and moves the issue to the configured review state. The orchestrator keeps inspecting the PR after publication: already-open Symphony PRs are rediscovered after restart, passing or pending checks remain in human review, merged/closed PRs are recorded as terminal, and failing checks or requested changes requeue the issue with a focused follow-up prompt. GitHub credentials stay in the orchestrator process and are not mounted into worker containers.
